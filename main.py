...
def dash():
    if HasPrivilege(session["AccountId"]):
        ...
        if User["LastBuild"] == GetBuild():
            return render_template("dash.html", title="Dashboard", session=session, data=DashData(),
                                   plays=RecentPlays(), config=UserConfig, Graph=DashActData(),
...
def BanchoSettings():
    if HasPrivilege(session["AccountId"], 4):
        # no bypassing it.
        if request.method == "GET":
            return render_template("banchosettings.html", preset=FetchBSData(), title="Bancho Settings",
                                   data=DashData(), bsdata=FetchBSData(), session=session, config=UserConfig)
        if request.method == "POST":
            try:
...
@app.route("/rank/<mapsetID>", methods=["GET", "POST"])
def RankMap(mapsetID):
    global postcount
    if HasPrivilege(session["AccountId"], 3):
        if request.method == "GET":
            error = None
            beatmapInfo = GetBmapInfo(mapsetID)
            banchoDiffCount = CheckDiffCount(mapsetID)

            if banchoDiffCount == 0:
                error = "Mapset Not Found. (Even on bancho!)"
            elif beatmapInfo[0]["BeatmapId"] == -1:
                error = "DB doesn't have information of this beatmap but bancho says the map exists. Please load map on the client!"
            elif len(beatmapInfo) != banchoDiffCount:
                error = "Requested mapset's difficulty count doesn't matches with bancho's data! Did you make sure that every difficulties are loaded to DB?"

            return render_template("beatrank.html", title="Rank a Beatmapset!", data=DashData(), session=session,
                                   beatdata=GetBmapInfo(mapsetID), error=error, config=UserConfig, Id=mapsetID)
        if request.method == "POST":
            try:
                RankBeatmaps(mapsetID, request.form.to_dict())
                return render_template("beatrank.html", title="Rank a Beatmapset!", data=DashData(), session=session,
                                       beatdata=GetBmapInfo(mapsetID), config=UserConfig,
                                       success="Successfully updated state!", Id=mapsetID)
            except Exception as e:
                print(e)
                ConsoleLog(f"Error while ranking beatmapset ({mapsetID})!", f"{e}", 3)
                return render_template("beatrank.html", title="Rank a Beatmapset!", data=DashData(), session=session,
                                       beatdata=GetBmapInfo(mapsetID), config=UserConfig,
...
def RankFrom():
    if request.method == "GET":
        if HasPrivilege(session["AccountId"], 3):
            return render_template("rankform.html", title="Rank a Beatmapset!", data=DashData(), session=session,
                                   config=UserConfig)
        else:
            return NoPerm(session)
    else:
        if not HasPrivilege(session["AccountId"]):  # mixing things up eh
            ...
            if request.form['isBeatmapSet'] == "1":
                return redirect(f"/rank/{request.form['mapsetID']}")
            elif request.form['isBeatmapSet'] == "0":
                mapsetID = MapIdToMapsetID(request.form['mapID'])
                return redirect(f"/rank/{mapsetID}")
    ...
    if HasPrivilege(session["AccountId"], 6):
        if request.method == "GET":
            return render_template("users.html", title="Users", data=DashData(), session=session, config=UserConfig,
                                   UserData=FetchUsers(int(page) - 1), page=int(page))
        if request.method == "POST":
            return render_template("users.html", title="Users", data=DashData(), session=session, config=UserConfig,
                                   UserData=FindUserByUsername(request.form["user"], int(page)), page=int(page),
...
@app.route("/rankrq", methods=["GET", "POST"])
def RankFromRQ():
    if request.method == "GET":
        if HasPrivilege(session["AccountId"], 3):
            return render_template("rankrq.php", title="Rank Request List", data=DashData(), session=session,
                                   config=UserConfig)
        else:
            return NoPerm(session)
    else:
        if not HasPrivilege(session["AccountId"]):  # mixing things up eh
            ...
            return redirect(f"/rankrq/")  # does this even work
...
def SystemSettings():
    if HasPrivilege(session["AccountId"], 4):
        if request.method == "GET":
            return render_template("syssettings.html", data=DashData(), session=session, title="System Settings",
                                   SysData=SystemSettingsValues(), config=UserConfig)
        if request.method == "POST":
            try:
                ApplySystemSettings([request.form["webman"], request.form["gameman"], request.form["register"],
                                     request.form["globalalert"], request.form["homealert"]],
                                    session)  # why didnt i just pass request
                return render_template("syssettings.html", data=DashData(), session=session, title="System Settings",
                                       SysData=SystemSettingsValues(), config=UserConfig,
...
def EditUser(id):
    if request.method == "GET":
        if HasPrivilege(session["AccountId"], 6):
            return render_template("edituser.html", data=DashData(), session=session, title="Edit User",
                                   config=UserConfig, UserData=UserData(id), Privs=GetPrivileges(),
                                   UserBadges=GetUserBadges(id), badges=GetBadges())
        else:
            return NoPerm(session)
    if request.method == "POST":
        if HasPrivilege(session["AccountId"], 6):
            try:
                ...
                return render_template("edituser.html", data=DashData(), session=session, title="Edit User",
                                       config=UserConfig, UserData=UserData(id), Privs=GetPrivileges(),
                                       UserBadges=GetUserBadges(id), badges=GetBadges(),
                                       success=f"User {request.form['username']} has been successfully edited!")
            except Exception as e:
                print(e)
...
def Logs(page):
    if HasPrivilege(session["AccountId"], 7):
        return render_template("raplogs.html", data=DashData(), session=session, title="Logs", config=UserConfig,
                               Logs=RAPFetch(page), page=int(page))
    else:
        return NoPerm(session)
...
def ConfirmDelete(id):
    ...
    if HasPrivilege(session["AccountId"], 6):
        ...
        return render_template("confirm.html", data=DashData(), session=session, title="Confirmation Required",
                               config=UserConfig, action=f"delete the user {AccountToBeDeleted['Username']}",
                               yeslink=f"/actions/delete/{id}", backlink=f"/user/edit/{id}")
    else:
        return NoPerm(session)
...
def IPUsers(ip):
    if HasPrivilege(session["AccountId"], 6):
        ...
        return render_template("iplookup.html", data=DashData(), session=session, title="IP Lookup", config=UserConfig,
                               ipusers=IPUserLookup, IPLen=UserLen, ip=ip)
    else:
        return NoPerm(session)
...
def Badges():
    if HasPrivilege(session["AccountId"], 4):
        return render_template("badges.html", data=DashData(), session=session, title="Badges", config=UserConfig,
                               badges=GetBadges())
    else:
        return NoPerm(session)
...
def EditBadge(BadgeID: int):
    if HasPrivilege(session["AccountId"], 4):
        if request.method == "GET":
            return render_template("editbadge.html", data=DashData(), session=session, title="Edit Badge",
                                   config=UserConfig, badge=GetBadge(BadgeID))
        if request.method == "POST":
            try:
...
def EditPrivileges():
    if HasPrivilege(session["AccountId"], 13):
        return render_template("privileges.html", data=DashData(), session=session, title="Privileges",
                               config=UserConfig, privileges=GetPrivileges())
    else:
        return NoPerm(session)
...
def EditPrivilege(Privilege: int):
    if HasPrivilege(session["AccountId"], 13):
        if request.method == "GET":
            return render_template("editprivilege.html", data=DashData(), session=session, title="Privileges",
                                   config=UserConfig, privileges=GetPriv(Privilege))
        if request.method == "POST":
            try:
                ...
                return render_template("editprivilege.html", data=DashData(), session=session, title="Privileges",
                                       config=UserConfig, privileges=Priv,
...
def Console():
    if HasPrivilege(session["AccountId"], 14):
        return render_template("consolelogs.html", data=DashData(), session=session, title="Console Logs",
                               config=UserConfig, logs=GetLog())
    else:
        return NoPerm(session)
...
def ChangeLogs():
    if HasPrivilege(session["AccountId"]):
        return render_template("changelog.html", data=DashData(), session=session, title="Console Logs",
                               config=UserConfig, logs=Changelogs)
    else:
        return NoPerm(session)
...
def Admins():
    if HasPrivilege(session["AccountId"]):
        return render_template("admins.html", data=DashData(), session=session, title="Admins", config=UserConfig,
...
def ChangePass(AccountID):
    if HasPrivilege(session["AccountId"], 6):  # may create separate perm for this
        if request.method == "GET":
            ...
            return render_template("changepass.html", data=DashData(), session=session,
                                   title=f"Change the Password for {User['Username']}", config=UserConfig, User=User)
        if request.method == "POST":
            ChangePWForm(request.form)
            User = GetUser(int(AccountID))
            RAPLog(session["AccountId"],
                   f"has changed the password of {User['Username']} ({AccountID}) {request.form['time']}.")
...
def DonorAward(AccountID):
    if HasPrivilege(session["AccountId"], 6):
        if request.method == "GET":
            User = GetUser(int(AccountID))
            return render_template("donoraward.html", data=DashData(), session=session,
                                   title=f"Award Donor to {User['Username']}", config=UserConfig, User=User)
        if request.method == "POST":
            GiveSupporterForm(request.form)
            User = GetUser(int(AccountID))
            RAPLog(session["AccountId"],
                   f"has awarded {User['Username']} ({AccountID}) {request.form['time']} months of donor.")
            return redirect(f"/user/edit/{AccountID}")
    else:
        return NoPerm(session)
...
def ApiStatus():
    return jsonify(requests.get(
        UserConfig["ServerURL"] + "api/v1/users/rxfull?id=1000").json())  # this url to provide a predictable result


@app.route("/js/status/lets")
def LetsStatus():
    return jsonify(
        requests.get(UserConfig["LetsAPI"] + "v1/pp?b=1058295").json())  # this url to provide a predictable result


@app.route("/js/status/bancho")
def BanchoStatus():
    return jsonify(requests.get(
        UserConfig["BanchoURL"] + "api/v1/isOnline?id=1000").json())  # this url to provide a predictable result
